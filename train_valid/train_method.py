...
def sample_train(model, token, data, criterion, optimize, n_epoch, device, model_path=None):
    ...
    for i in range(n_epoch):
        ...
        test_f1, test_micro_f1 = dataset_valid(model, token, data['val'], device, criterion=criterion)
        print('Epoch: %d \t macro_f1: %.4f \t micro_f1: %.4f' % (i, test_f1, test_micro_f1))
        if test_f1 > best_model_f1:
            print('Val F1: %.4f > %.4f Saving Model' % (test_f1, best_model_f1))
