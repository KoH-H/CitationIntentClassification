...
def main_rev(path, dev):
    ...
    data = load_data(batch_size=8, dataname='ACL', radio=0.8)
    best_model_f1 = rev_train(model, token, data, criterion, optimizer, n_epoch, dev, model_path=path,
                              scheduler=scheduler)
...
def main_mul(path, dev):  # two task
    ...
    c_matrix = confusion_matrix(test_true, test_pre, labels=[0, 1, 2, 3, 4, 5])
    per_eval = classification_report(test_true, test_pre, labels=[0, 1, 2, 3, 4, 5])
    log_result(test_f1, best_model_f1,  c_matrix, per_eval, lr=lr, epoch=n_epoch, fun_name='main_mul')
...
def main_sci(path, dev):
    ...
    c_matrix = confusion_matrix(test_true, test_pre, labels=[0, 1, 2, 3, 4, 5])
    per_eval = classification_report(test_true, test_pre, labels=[0, 1, 2, 3, 4, 5])
    log_result(test_f1, best_model_f1, c_matrix, per_eval, lr=lr, epoch=n_epoch, fun_name='main_sci')
